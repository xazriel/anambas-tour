<div class="space-y-6" x-data="{ open: @entangle('showModal') }">
    <div class="flex justify-between items-center">
        <h2 class="text-xl font-bold text-slate-800">Manage Culinaries</h2>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
        <div class="p-4 border-b border-slate-100">
            <input wire:model.live="search" type="text" placeholder="Search culinary..." class="w-full md:w-80 rounded-xl border-slate-200 focus:ring-indigo-500">
        </div>
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition">
                    ...
                    <td class="px-6 py-4 text-right space-x-2">
                        ...
                        <button wire:click="delete({{ $item->id }})" wire:confirm="Delete this item?" class="text-rose-600 hover:text-rose-900 font-medium">Delete</button>
                    </td>
                </tr>
                @empty
                <tr><td colspan="3" class="px-6 py-10 text-center text-slate-400 italic">No culinaries found.</td></tr>
                @endforelse
            </tbody>
        </table>
        <div class="p-4 bg-slate-50">{{ $items->links() }}</div>
    </div>
    ...
    <div x-show="open" class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
        ...
        <div class="bg-white rounded-3xl shadow-2xl z-50 w-full max-w-2xl p-8 transform transition-all">
            <h3 class="text-2xl font-black text-slate-800 mb-6 uppercase tracking-tight">{{ $isEditMode ? 'Edit' : 'Add' }} Culinary</h3>
