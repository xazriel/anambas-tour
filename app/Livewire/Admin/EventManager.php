...
class EventManager extends Component
{
    ...
    public function edit($id) {
        ...
        $this->selected_id = $id;
        $record = \App\Models\Event::findOrFail($id);
    ...
    public function store() {
        ...
        if ($this->isEditMode) {
            \App\Models\Event::find($this->selected_id)->update($data);
        } else {
    ...
    public function delete($id) {
        $record = \App\Models\Event::find($id);
    ...
    public function render() {
        return view('livewire.admin.event-manager', [
            'items' => \App\Models\Event::where('name', 'like', '%' . $this->search . '%')
                ->latest()->paginate(10)
        ]);
    }
}